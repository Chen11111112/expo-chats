...
import { Alert, Image, Platform, ScrollView, StyleSheet, TextInput, TouchableOpacity } from 'react-native';
...
export default function IGProfileScreen() {
  ...
  const [username, setUsername] = useState('shadow_dev_2026');
      ...
      <ScrollView contentContainerStyle={{ alignItems: 'center' }}>
        ...
        <View style={styles.formContainer}>
          <View style={styles.inputRow}>
            ...
            <TextInput style={styles.input} value={name} onChangeText={setName} placeholder="姓名" />
          </View>
          <View style={styles.inputRow}>
            <Text style={styles.label}>用戶名稱</Text>
            <TextInput style={styles.input} value={username} onChangeText={setUsername} placeholder="用戶名稱" />
          </View>
          <View style={styles.inputRow}>
            <Text style={styles.label}>簡介</Text>
            <TextInput style={styles.input} placeholder="寫點什麼吧..." multiline />
          </View>
        </View>

        <TouchableOpacity style={styles.saveButton} onPress={() => Alert.alert("成功", "設定已儲存")}>
...
const styles = StyleSheet.create({
  ...
  changePhotoText: { color: '#0095f6', fontSize: 14, fontWeight: '600', textAlign: 'center' },
